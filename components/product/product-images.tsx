...
export default function ProductImages({ images, productName, selectedVariantImages }: ProductImagesProps) {
  const [selectedIndex, setSelectedIndex] = useState(0)
  
  // Usar imagens da variação se disponíveis, caso contrário usar imagens do produto
  const displayImages = selectedVariantImages && selectedVariantImages.length > 0 
    ? selectedVariantImages 
    : images

  // Resetar índice quando as imagens mudarem (ex: trocou variação)
  useEffect(() => {
    setSelectedIndex(0)
  }, [selectedVariantImages])
