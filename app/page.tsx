...
export default async function Home() {
  let mainBanner: any = null
  try {
    mainBanner = await prisma.banner.findFirst({
      where: { active: true, isMain: true },
      orderBy: { order: 'asc' },
    })
  } catch {}

  let carouselBanners: any[] = []
  try {
    carouselBanners = await prisma.banner.findMany({
      where: { active: true, isMain: false },
      orderBy: { order: 'asc' },
    })
  } catch {}

  let carouselCategories: any[] = []
  try {
    carouselCategories = await prisma.category.findMany({
  ...
  try {
    promotionProducts = await prisma.product.findMany({
      where: { active: true },
      include: { category: true },
      take: 6,
      orderBy: { createdAt: 'desc' },
