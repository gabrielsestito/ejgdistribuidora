...
async function getCoordinatesFromZipCode(zipCode: string): Promise<CoordinatesResult> {
  ...
  // Fallback: tentar BrasilAPI para obter cidade/UF quando ViaCEP falhar
  if (!viaCepData) {
    try {
      const br = await fetchJsonWithTimeout(
        `https://brasilapi.com.br/api/cep/v1/${cleanZipCode}`,
        {},
        6000
      )
      // Normalizar campos no formato parecido com ViaCEP
      if (br && (br as any).city && (br as any).state) {
        viaCepData = {
          logradouro: (br as any).street || '',
          bairro: (br as any).neighborhood || '',
          localidade: (br as any).city,
          uf: (br as any).state,
        }
      }
    } catch {
      // continua sem viaCepData
    }
  }
  ...
  // Fallback de geocoding: Open-Meteo (sem chave), usando cidade/UF
  try {
    ...
    if (city && state) {
      const fallback = await fetchJsonWithTimeout(
        `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(`${city}, ${state}, Brazil`)}&count=1&language=pt&format=json`,
        {},
        6000
      )
      if (fallback && Array.isArray(fallback.results) && fallback.results.length > 0) {
        return {
          coordinates: {
            lat: parseFloat(fallback.results[0].latitude),
            lng: parseFloat(fallback.results[0].longitude),
          },
        }
      }
    }
  } catch {
    // ignora e cai nos retornos padrão
  }
